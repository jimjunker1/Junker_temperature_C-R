...
##' @param prod_seasons
##' 
create_diet_matrices <- function(diet_df, prod_seasons) {
  ...
  create_matrix = function(single_diet_df, name_order){
    ...
    filled_long_df = plyr::join(full_long_df, res_long_df, by = c("taxon", "diet_item")) %>% dplyr::mutate(rel_area = tidyr::replace_na(rel_area, 0))
  ...
  # set the name order
  name_list = prod_seasons %>% purrr::map(~.x %>% pluck(1) %>% dplyr::select(taxon_id) %>% unique %>% unlist) %>% 
